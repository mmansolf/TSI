...
out.sep=list()
for(separated in c(F,T)){
  library(mice)
  pct=proc.time()
  mice.data=mice(useData,m=10,maxit=10,
                 method=mice.method,
                 blocks=mice.blocks,
  ...
  for(j in 1:nsim){
    ...
    smice.data=fact.mice(trueCorData2,m=10,maxit=10,
                         method=mice.method,
                         blocks=mice.blocks,
                         blots=mice.blots%>%map(~map(.,function(x)c(x,list(separated=separated)))),
                         predictorMatrix=mice.predMx,
                         ...
                         printFlag=F)
    smice.data2=fact.mice(trueCorData2,m=10,maxit=10,
                          method=mice.method,
                          blocks=mice.blocks,
                          blots=mice.blots%>%map(~map(.,function(x)c(x,list(separated=separated)))),
  ...
  out.sep[[as.character(separated)]]=tibble(
    ...
    simHUI=list(simHUI%>%map(~.$smice.data)),
    simHUI2=list(simHUI%>%map(~.$smice.data2)),
...
allOut2pc=allOut2%>%
  ...
  select(items,contains('Mean'),contains('SD'))%>%
